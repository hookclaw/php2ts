...
/// <reference path="../typings/argv.d.ts" />
/// <reference path="../typings/typescript.1.8.10.d.ts" />
...
import compiler2pass = require('../share/php2ts-compiler2pass');
...
let compile = (resultStok:resultStok,startFolder:string,settings:Settings,constants:Constants):void => {
    ...
    //     // composer.json
    //     let json_file = startFolder + '/' + COMPOSER_JSON;
    //     if(fs.existsSync(json_file)) {
    //         let json = fs.readFileSync(json_file);
    //         let composer = JSON.parse(json.toString());
    //         autoload = (composer['autoload'] === undefined)?{}:composer['autoload'];
    //         let autoloadDev = (composer['autoload-dev'] === undefined)?{}:composer['autoload-dev'];
    //         for(let key1 in autoloadDev) {
    //             if(autoload[key1] === undefined) {
    //                 autoload[key1] = {};
    //             }
    //             for(let key2 in autoloadDev[key1]) {
    //                 autoload[key1][key2] = autoloadDev[key1][key2];
    //             }
    //         }
    //     }
    // }
    ...
    //     let ast = target.result.body.ast;
    //     let relativePath = path.relative(startFolder,target.file);
    //     compiler2pass.pass1(ast,relativePath);
    // }
    // // pass2
    // for(let target of resultStok) {
    //     let ast = target.result.body.ast;
    //     // compile
    //     let result = compiler2pass.pass2(ast,target.result.body.comments);
    //     //console.dir(result);
    //     // write
    //     let parsedPath = path.parse(target.file);
    //     parsedPath.ext = '.ts';
    //     parsedPath.base = parsedPath.name + parsedPath.ext;
    //     let tsFilename = path.format(parsedPath);
    //     let formattedText = formatter.formatTypeScriptCode(tsFilename,result.code);
    //     fs.writeFileSync(tsFilename,formattedText);
    //     console.log('create -> '+tsFilename);
    // }
}
