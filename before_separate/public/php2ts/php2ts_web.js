...
function setup_translate() {
    var content = document.getElementById("code"), result = document.getElementById("result"), 
    // ast_dump = <HTMLElement>document.getElementById("ast_dump"),
    php_code = document.getElementById("php_code"), ts_code = document.getElementById("ts_code"), 
    // js_code = <HTMLElement>document.getElementById("js_code"),
    transpile_result = document.getElementById("transpile_result"), loc = window.location, run = function (e) {
        try {
            if (e !== undefined) {
                e.preventDefault();
            }
...
function php2ts(code, callback) {
    compile_phpfile(code, function (ok, compile_result) {
        ...
        try {
            var compilerOptions = { diagnostics: true, allowUnusedLabels: false };
            var formatResult = formatTypeScriptCode(ret.compile_result, '', bufferedReadFileSyncTS, compilerOptions);
...
//# sourceMappingURL=php2ts_web.js.map